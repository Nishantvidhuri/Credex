...
function ChatWidget({ darkMode }) {
  ...
  // API keys - accessed from environment variables only
  const openAIKey = import.meta.env.VITE_OPENAI_API_KEY || '';
  const geminiAPIKey = import.meta.env.VITE_GEMINI_API_KEY || '';

  // Use Gemini API instead of OpenAI - can be configured via env variable
  const useGeminiAPI = import.meta.env.VITE_USE_GEMINI_API === "true" || true;
  ...
  return (
    <div className="chat-widget">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-20 right-5 md:right-10 bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-sm overflow-hidden border border-gray-200 dark:border-gray-700"
          >
            {/* Chat header */}
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-100">
                    {useGeminiAPI ? "Powered by Gemini" : "Powered by OpenAI"}
                  </p>
                </div>
              </div>
              <button 
      ...
      <motion.button
        ...
        className="fixed bottom-5 right-5 md:right-10 bg-blue-600 text-white p-4 rounded-full shadow-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors"
